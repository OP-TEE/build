...
run-only:
	@test -f $(BINARIES_PATH)/bl31.elf || \
		(echo "Error: bl31.elf missing: run 'make all'." && exit 1)
	@test -f $(BINARIES_PATH)/tee-raw.bin || \
		(echo "Error: tee-raw.bin missing: run 'make all'." && exit 1)
	@test -f $(BINARIES_PATH)/u-boot.elf || \
		(echo "Error: u-boot.elf missing: run 'make all'." && exit 1)
	@test -f $(BINARIES_PATH)/versal2-vek385-revA.dtb || \
		(echo "Error: versal2-vek385-revA.dtb missing: run 'make all'." && exit 1)
	@test -f $(BOOTGEN_BIN) || \
		(echo "Error: bootgen binary missing: run 'make all'." && exit 1)
	@test -f $(QEMU_AARCH64_BIN) || \
		(echo "Error: $(QEMU_AARCH64_BIN) missing: run 'make all'." && exit 1)
	@test -f $(QEMU_MICROBLAZE_BIN) || \
		(echo "Error: $(QEMU_MICROBLAZE_BIN) missing: run 'make all'." && exit 1)
	@test -f $(QEMU_RISCV_BIN) || \
		(echo "Error: $(QEMU_RISCV_BIN) missing: run 'make all'." && exit 1)
	@test -f $(DEVICE_TREE_PATH)/board-versal2-pmxc-virt.dtb || \
		(echo "Error: pmxc-virt.dtb missing: run 'make all'." && exit 1)
	@test -f $(DEVICE_TREE_PATH)/board-versal2-asu-virt.dtb || \
		(echo "Error: asu-virt.dtb missing: run 'make all'." && exit 1)
	@test -f $(DEVICE_TREE_PATH)/board-versal2-psxc-vek385.dtb || \
		(echo "Error: psxc-vek385.dtb missing: run 'make all'." && exit 1)
	@test -f $(BINARIES_PATH)/Image || \
		(echo "Error: Image missing: run 'make all'." && exit 1)
	@test -f $(BINARIES_PATH)/rootfs.cpio.gz.u-boot || \
		(echo "Error: rootfs image missing: run 'make all'." && exit 1)
	$(MAKE) bootimage run-qemu-direct
...
TF_A_FLAGS = PLAT=versal2 CONSOLE=pl011 RESET_TO_BL31=1 SPD=opteed \
	     CUSTOM_PKG_PATH="$(SCMI_PATH)" DEBUG=0 \
